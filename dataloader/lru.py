...
class LRUDataloader(AbstractDataloader):
    ...
    def _get_train_loader(self, infinite=False):
        ...
        if not infinite:
            return data_utils.DataLoader(dataset, shuffle=True, pin_memory=True,
                                         batch_size=self.args.train_batch_size, num_workers=self.args.num_workers)
        else:
            return data_utils.DataLoader(dataset, sampler=InfiniteSampler(dataset),  pin_memory=True, 
                                         batch_size=self.args.train_batch_size, num_workers=self.args.num_workers)
